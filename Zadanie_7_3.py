...
class BaseContact:
    ...
    def label_length(self):
        return int(len(self.first_name) + len(self.last_name))
...
def create_card(card_type, quantity):
    ...
    if card_type == "C":
        for i in range(quantity):
            card = BaseContact(first_name = fake.first_name(), last_name = fake.last_name(), phone_number = fake.phone_number(), email = fake.free_email())
            cards.append(card) 
    elif card_type == "B":
        for i in range(quantity):
            card = BaseContact(first_name = fake.first_name(), last_name = fake.last_name(), phone_number = fake.phone_number(), fake_company = fake.company(), fake_job = fake.job(), email = fake.free_email())
            cards.append(card) 
    else:
